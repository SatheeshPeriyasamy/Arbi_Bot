...
async def fetch_futures_prices(symbol):
    async with aiohttp.ClientSession() as session:
        async with session.get(f"{BINANCE_FUTURES_API_URL}/ticker/price?symbol={symbol}") as response:
...
def adjust_quantity_for_lot_size(symbol, quantity):
    # Default lot values if not fetched
...
async def execute_arbitrage(client, symbol, spot_price, futures_price):
    min_notional_value = 10 #Mentioned in USDT
    quantity = 0.01  
    ...
    try:
        if spot_price < futures_price:
            logging.debug(f"Attempting to execute arbitrage: Buy {symbol} on spot at {spot_price}, Sell on futures at {futures_price}")
            ...
            logging.debug(f"Attempting to execute reverse arbitrage: Sell {symbol} on spot at {spot_price}, Buy on futures at {futures_price}")
...
async def check_and_execute_arbitrage(client, symbol):
    if price_dict[symbol]['spot'] is not None and price_dict[symbol]['futures'] is not None:
        logging.debug(f"Checking arbitrage opportunity for {symbol.upper()}: spot price {price_dict[symbol]['spot']}, futures price {price_dict[symbol]['futures']}")
        await execute_arbitrage(client, symbol.upper(), price_dict[symbol]['spot'], price_dict[symbol]['futures'])
        price_dict[symbol] = {'spot': None, 'futures': None} #Resetting of prices

async def main():
    client = await AsyncClient.create('your_spot_api_key', 'your_spot_api_secret')
    await client.futures_create('your_futures_api_key', 'your_futures_api_secret')

    await fetch_lot_and_step_sizes()  #Fetch lot and step sizes to avoid Notional Errors
